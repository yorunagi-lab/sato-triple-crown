...
- POST `/api/cheers`：UUID v4の配列 `eventIds`（1〜40件）を受け取り、保存した累計 `total` と新規受付件数 `accepted` を返します。旧形式の単独 `eventId` にも対応します。
- GET `/health`：Workerの応答確認。DBの読み書き確認には `/api/cheers` を使います。
- 受付IDと累計を一つのトランザクションで保存。24時間以内の同じ受付IDによる再送を重複加算しません。
- 通常の連打は1回ずつ集計します。同じIP由来で10秒間に200件を超える新規エールは制限します。複数タップを一括送信してリクエスト数を抑えます。IPの日次ハッシュはメモリー上だけで保持し、DBにIPや利用者IDは保存しません。Workerのログ収集はこの構成では無効です。
- 許可Originは `https://yorunagi-lab.github.io`。`wrangler.jsonc` の `ALLOWED_ORIGIN` で変更できます。Origin確認はボットの完全な排除を保証するものではありません。
- 人数ではなく、受け付けたエールの累計です。通信結果が不明なときは同じ受付IDの配列で1回だけ再試行します。429の場合は指定された待機時間を置きます。
...
ローカルSQLiteによる同時更新・重複防止・再起動時の保持・書込失敗時の巻戻し・一括送信の重複防止など10テストと、Workerのビルドを確認しました。ブラウザでは模擬APIを使って共有表示・安全な再試行・失敗表示を確認しています。Cloudflare上の実際の永続化と複数端末での集計は、配置後に確認します。
