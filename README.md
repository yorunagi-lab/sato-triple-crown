...
- 初回公開ではGitHub Actionsで14テスト、実データの取得・検証・保存、Pages公開に成功しました。現在のテストは共有カウンターの7件を加えた21件です。
...
「エールを送る」を押すと、オリジナルのSVGゴリラが下から登場し、左右の腕でドラミング。紙吹雪と応援メッセージを表示します。音は出ません。端末の「視差効果を減らす」設定では静止ポーズに切り替えます。
...
- 共有カウントの単位は受け付けたエールの回数です。人数ではなく、同じ人が複数回押すこともできます。演出中は再送を防ぎ、短時間の連打をサーバー側でも制限します。
...
順位・打率の厳密比較・同順位・試算・取得失敗時の保持・取得停止・日時解釈の14テストに加え、共有カウンターの同時更新・重複防止・再起動後の保持・書込失敗の巻戻し・連打制限・入力検証・HTTP境界の7テストを用意しています。カウンターの保存テストはNode.jsのSQLiteを用いたローカル検証です。

Chromiumで画面幅320・393・768・1440pxの表示、横はみ出しなし、エール演出、端末内カウントの再読込後の保持を確認しました。共有APIの模擬応答で、別ページへの累計反映、再送時の受付ID維持、送信失敗時に加算しないこと、動きを減らす設定を確認しました。スマホ実機での確認は未実施です。
